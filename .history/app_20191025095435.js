...
app.post('/event_create', (req,res) => {
    ...
    const queryString = "INSERT INTO event (Event_Name, Event_Date, Event_Time, Event_Location) VALUES(?, ?, ?, ?)";
    getConnection().query(queryString,[event_name,event_date, event_time, event_location],(err,results,fields) =>{
        if(err){
...
app.get('/upcoming_events', (req, res) => {
    ...
    const queryString = "SELECT Event_Name, MAX(Event_Date) AS Event_d, Event_Time, Event_Location FROM event WHERE Event_Date > " + Date.now();
...
app.get('/events', (req, res) => {
    ...
    const queryString = "SELECT Event_Name, Event_Date AS Event_d, Event_Time, Event_Location FROM event";
    getConnection().query(queryString, (err,rows, fields) =>{
        if(err){
            console.log("Failed to query for users: " + err);
            res.sendStatus(500)
